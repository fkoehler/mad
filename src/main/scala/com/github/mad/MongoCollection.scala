...
class MongoCollection(val underlying: MadMongoCollection) extends SyncCollection with AsyncCollection
with ManagedSyncCollection with ManagedAsyncCollection {
...
trait SyncCollection {
  val underlying: MadMongoCollection
  ...
  def findOne(find: MadBuilder): Option[BsonDoc] = Option(underlying.findOne(find))

  def findOneAs[T](doc: BsonDoc)(implicit c: FromBsonDoc[T]): Option[T] = findOneAs(Find().query(doc))

  def findOneAs[T](find: MadBuilder)(implicit c: FromBsonDoc[T]): Option[T] = findOne(find).map(c.fromBson)
...
trait AsyncCollection extends AsyncHelpers {
  ...
  def findOneAsync(doc: BsonDoc): Future[Option[BsonDoc]] =
    wrapCallback[Document]((cb: Callback[Document]) => underlying.findOneAsync(cb, doc)).map(Option(_))

  def findOneAsyncAs[T](doc: BsonDoc)(implicit c: FromBsonDoc[T]): Future[Option[T]] =
    wrapCallback[Document]((cb: Callback[Document]) => underlying.findOneAsync(cb, doc)).map(document => Option(document) match {
  ...
  def findAsync(query: BsonDoc): Future[MongoIterator] =
    wrapCallback[MadMongoIterator[Document]]((cb: Callback[MadMongoIterator[Document]]) => underlying.findAsync(cb, query)).map(MongoIterator(_))
...
trait ManagedAsyncCollection extends AsyncHelpers {
  val underlying: MadMongoCollection

  def managedFindAsync(query: BsonDoc): Future[ManagedMongoIterator] =
    wrapCallback[MadMongoIterator[Document]]((cb: Callback[MadMongoIterator[Document]]) => underlying.findAsync(cb, query)).map(iter => ManagedMongoIterator(managed(MongoIterator(iter))))
...
trait AsyncHelpers {
  def wrapCallback[T](opExecutor: (Callback[T]) => Unit): Future[T] = {
